...
//my code begins
static void push_to_stack(char *file_name,void **esp,char **save_ptr);
...
process_wait (tid_t child_tid) 
{
	...
	for(itr=list_begin(&thread_current()->child_proc);itr!=list_end(&thread_current()->child_proc);itr=list_next(itr))
	{
	...
	if(!ch || !found)
		return -1;
	thread_current()->waiting_on_thread=ch->tid;    
	if(!ch->used)
	  	sema_down(&thread_current()->child_lock);
	list_remove(found);
...
//my code begins
//pushing arguments to the stack in the format asked
static void push_to_stack(char *file_name,void **esp,char **save_ptr)
{
	//printf("pushing into stack as the format required for pintos\n");
    	char *token=file_name;
    	void *stack_pointer=*esp;
   	int argc=0;
    	int total_length=0;
	//pushing the different tokens
	while(token!=NULL)
	{
		//printf("%s\n",token);
		...
		//printf("%s!!",(char *)(stack_pointer));
		argc++;
	}

	char *args_pointer=(char *)stack_pointer;

	//push word_align attribute
	int  word_align=0;
	...
	while(args_pushed<argc)
	{
		...
		*((char **) stack_pointer)=args_pointer;
		//printf("%s adding char** \n",*((char **)stack_pointer));
		args_pushed++;
		args_pointer+=(strlen(args_pointer)+1);
	}

	//push char** in stack
	char **first_arg=(char **)stack_pointer;
        stack_pointer-=sizeof(char **);
        *((char ***)stack_pointer)=first_arg;

	//push number of arguments in stack
	stack_pointer-=sizeof(int);
	*(int *)(stack_pointer)=argc;

	//push return address in stack
	stack_pointer-=sizeof(int*);
	*(int *)(stack_pointer)=0;
	*esp=stack_pointer;
}
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  if (!setup_stack (esp))
	...
	//my code begins
	push_to_stack(fn_cp,esp,&save_ptr);
  	free(fn_cp);
	//my code ends
  /* Start address. */
